...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket):
        ...
        if user_id and room_id in self.active_connections:
    ...
    async def cleanup_stale_connections(self, max_idle_minutes: int = 60):
        ...
        for websocket in stale_connections:
            logger.info(f"Cleaning up stale connection from room {self.connection_metadata[websocket]['room_id']}")
            self.disconnect(websocket)
            try:
...
class CollaborationManager:
    ...
    async def notify_comment_added(self, room_id: str, comment_data: dict, exclude_user_id: Optional[int] = None):
        ...
        # Find WebSocket to exclude based on user ID
        exclude_websocket = None
        if exclude_user_id and room_id in self.connection_manager.active_connections:
